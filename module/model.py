...
from module.layer import GCNLayer, GraphSAGELayer
...
class GCNPPI_GCN(nn.Module):
    def __init__(self, in_feats, out_feats, part_size):
        super(GCNPPI_GCN, self).__init__()
...
class GCNPPI_SAGE(nn.Module):
    def __init__(self, in_feats, out_feats, part_size):
        ...
        self.dropout = nn.Dropout(p=0.1)
        self.activation = F.relu
        self.gsageLayer1 = GraphSAGELayer(in_feats=in_feats, out_feats=2048)
        self.gsageLayer2 = GraphSAGELayer(in_feats=2048, out_feats=2048)
        self.gsageLayer3 = GraphSAGELayer(in_feats=2048, out_feats=2048)
        self.gsageLayer4 = GraphSAGELayer(in_feats=2048, out_feats=2048)
        self.gsageLayer5 = GraphSAGELayer(in_feats=2048, out_feats=2048)
        self.linear1 = nn.Linear(2048, 2048)
        self.linear2 = nn.Linear(2048, out_feats)
        self.layer_norm1 = nn.LayerNorm(2048, elementwise_affine=True)
        self.layer_norm2 = nn.LayerNorm(2048, elementwise_affine=True)
    ...
    # def forward(self, graphStructure, subgraphFeature):
    def forward(self, g_strt, feat):
        logits = self.gsageLayer1.forward(g_strt, feat)
        logits = self.layer_norm1(logits)
        logits = self.activation(logits)
        
        logits = self.gsageLayer2.forward(g_strt, logits)
        logits = self.layer_norm2(logits)
        logits = self.activation(logits)
        
        logits = self.gsageLayer3.forward(g_strt, logits)
        logits = self.layer_norm3(logits)
        logits = self.activation(logits)
        
        logits = self.gsageLayer4.forward(g_strt, logits)
        logits = self.layer_norm4(logits)
        logits = self.activation(logits)
        
        logits = self.gsageLayer5.forward(g_strt, logits)
        logits = self.layer_norm5(logits)
        logits = self.activation(logits)
